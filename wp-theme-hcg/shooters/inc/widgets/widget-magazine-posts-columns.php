...
class WorldStar_Magazine_Posts_Columns_Widget extends WP_Widget {
	...
	function __construct() {
		...
		parent::__construct(
			...
			esc_html__( 'Magazine (Columns)', 'worldstar' ), // Name.
			array(
				'classname' => 'worldstar-magazine-columns-widget',
				'description' => esc_html__( 'Displays your posts from two selected categories.', 'worldstar' ),
	...
	private function default_settings() {
		$defaults = array(
			'category_one'       => 0,
			'category_two'       => 0,
			'category_one_title' => esc_html__( 'Left Category', 'worldstar' ),
			'category_two_title' => esc_html__( 'Right Category', 'worldstar' ),
			'number'             => 4,
			'highlight_post'     => true,
		);
		return $defaults;
	...
	function render( $args, $settings ) {
		// Get cached post ids.
		$post_ids_category_one = worldstar_get_magazine_post_ids( $this->id . '-left-category', $settings['category_one'], $settings['number'] );
		$post_ids_category_two = worldstar_get_magazine_post_ids( $this->id . '-right-category', $settings['category_two'], $settings['number'] );
		?>
	...
	function category_title( $args, $settings, $category_id, $category_title ) {
		...
		if ( ! empty( $widget_title ) ) :
			...
			$widget_title = worldstar_magazine_widget_title( $widget_title, $category_id );
