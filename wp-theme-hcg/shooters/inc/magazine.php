...
*/
function worldstar_magazine_widgets() {
	...
	elseif ( is_customize_preview() ) :
		...
		worldstar_customize_magazine_placeholder();
	elseif ( is_page_template( 'template-magazine.php' ) && current_user_can( 'edit_theme_options' ) ) :
		echo '<p class="empty-widget-area">';
		esc_html_e( 'Please go to Customize &#8594; Widgets and add at least one widget to the Magazine Homepage widget area.', 'worldstar' );
		echo '</p>';
	endif;
}
if ( ! function_exists( 'worldstar_magazine_widget_title' ) ) :
	/**
	 ...
	 */
	function worldstar_magazine_widget_title( $widget_title, $category_id ) {
		// Check if widget shows a specific category.
		if ( $category_id > 0 ) {
			// Set URL and Title for Category.
			$category_title = sprintf( esc_html__( 'View all posts from category %s', 'worldstar' ), get_cat_name( $category_id ) );
